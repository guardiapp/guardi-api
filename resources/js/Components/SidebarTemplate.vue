<template>
  <div>
    <!-- Desktop sidebar -->
    <aside class="z-20 hidden w-64 overflow-y-auto md:block flex-shrink-0" :class="{
      ...
      <div class="py-4" :class="{
        ...
        <Link class="ml-6 text-lg font-bold" href="/dashboard" :class="{
    ...
    <!-- Mobile sidebar -->
    <!-- Backdrop -->
    <div v-if="themeStore.isSideMenuOpen" transition:enter="transition ease-in-out duration-150"
      transition:enter-start="opacity-0" transition:enter-end="opacity-100"
      transition:leave="transition ease-in-out duration-150" transition:leave-start="opacity-100"
      transition:leave-end="opacity-0"
      class="fixed inset-0 z-10 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center"></div>
    <aside class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto md:hidden" :class="{
      'bg-white': !themeStore.dark,
      'bg-gray-800': themeStore.dark,
    }" v-show="themeStore.isSideMenuOpen" transition:enter="transition ease-in-out duration-150"
      transition:enter-start="opacity-0 transform -translate-x-20" transition:enter-end="opacity-100"
      transition:leave="transition ease-in-out duration-150" transition:leave-start="opacity-100"
      transition:leave-end="opacity-0 transform -translate-x-20" @click.away="themeStore.closeSideMenu"
      @keydown.escape="themeStore.closeSideMenu">
      <div class="py-4" :class="{
        'text-gray-500': !themeStore.dark,
        'text-gray-400': themeStore.dark,
      }">
        <a class="ml-6 text-lg font-bold" :class="{
          'text-gray-800': !themeStore.dark,
          'text-gray-200': themeStore.dark,
        }" href="#">
          Visitas
        ...
        <ul class="mt-6">
          <li class="relative px-6 py-3">
            <span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg"
              aria-hidden="true"></span>
            <Link class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150" :class="{
              'text-gray-800  hover:text-gray-800':
                !themeStore.dark,
              ' hover:text-gray-200 text-gray-100':
                themeStore.dark,
