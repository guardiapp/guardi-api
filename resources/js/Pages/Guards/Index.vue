<template>
    <MainLayout>
        <div class="container px-6 mx-auto grid">
            ...
            <TableTemplate
                :columns="['Nombre', 'Apellido', 'Residencia', 'Acciones']"
...
const transformedGuards = guards.map((guard) => ({
    ...
    residence: guard.residence.name, // Extract manager name
    actions: { id: guard.id }, // Store reference for actions
}));

const deleteGuard = (id) => {
    if (confirm("Are you sure you want to delete this guard?")) {
        axios.delete(`/guards/${id}`).then(() => {
